...
void debs_fifo_q1::FIFO_push_front(time_t expire_time, ROUTE_KEY_TYPE route_key)
{
    //make new route
...
ROUTE_KEY_TYPE debs_fifo_q1::FIFO_pop_back_if_expired()
{
    //check for empty list
    if(FIFO_tail==NULL) return -1;

    if(FIFO_tail->expire_time <= *current_time)
    {
        ROUTE_KEY_TYPE key = FIFO_tail->expire_time;
