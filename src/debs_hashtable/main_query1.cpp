...
int main(int argc, char** argv)
{
    ...
    //hashtable
    hashtable_counter *ht;
    bool *tops_changed;
    struct bucket **modified_bucket;
    ...
    //create hashtable
    ht = new hashtable_counter();
    tops_changed = ht->get_tops_changed_pointer();
    modified_bucket = ht->get_modified_bucket_pointer();
    *tops_changed = false;
    ...
    while(inFile.getline(line,2048) ) //&& i++<100
    {
        ...
        if(proc_line(line, &current_time, cells))
        {
            ...
            key = get_key(cells);
            hash_key = ht->hash_func(key);
            if(key<0) {cout<<"key problem"<<endl; cin>>key; return 0;}
            //add to hashtable
            c_ht_inc++; start=clock(); ht->force_inc_counter(hash_key, key, -1); ht_inc+=clock()-start;

            //push into FIFO
            c_FIFO_push++; start=clock(); FIFO->FIFO_push_front(current_time+MIN30TOSEC, *modified_bucket); FIFO_push+=clock()-start;
        }
        ...
        while( (temp_bucket=FIFO->FIFO_pop_back_if_expired()) != NULL)
        {
            FIFO_pop+=clock()-start;

            c_ht_dec++; start=clock(); ht->dec_counter_update_list(temp_bucket); ht_dec+=clock()-start;
